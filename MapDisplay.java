...
/**
 * Do not modify.
 * This class takes 2 argument - cityxy.txt citypairs.txt
 * This class is adapted from linanqiu.github.com
 * This class visualizes your dijkstra's algorithm in a map.
 */
...
public class MapDisplay extends JFrame {
  ...
  public static void main(String[] args) {
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        try {
          MapDisplay frame = new MapDisplay(args[0], args[1]);
  ...
  public static Graph readGraph(String vertexFile, String edgeFile) {
	  ...
	  try {
	      ...
	      vertexFileBr.close();
	      System.out.println(dijkstra.vertices);
	      ...
	      while ((line = edgeFileBr.readLine()) != null) {
		        ...
		        System.out.println(parts[0]); //tester
		        System.out.println(parts[1]); //tester
		        System.out.println(Double.parseDouble(parts[2])); //tester
   ...
   * Create the frame.
   */
  public MapDisplay(String vertexFile, String edgeFile) {
    setTitle("U.S Roadtrips: Shortest Route Visualizer");
    ...
    JLabel lblPanda = new JLabel("Adapted from http://linanqiu.github.io/");
    ...
    updateGraphPanel(vertexFile, edgeFile);
  }

  private void updateGraphPanel(String vertexFile, String edgeFile) {
  ...
  public class GraphPanel extends JPanel {
    ...
    public Graph dijkstra;

    public HashMap<String, List<Edge>> overlayEdges;

    public GraphPanel(Graph dijkstra) {
    ...
    public void paintGraph(Graphics g) {
      for (Vertex v : dijkstra.getVertices()) {
        for (Edge edge : v.neighbors) {
          paintEdge(g, edge.source, edge.dest, edge.weight, Color.LIGHT_GRAY, DEFAULT_THICKNESS, 255);
        }
      }
      for (Vertex v : dijkstra.getVertices()) {
        paintVertex(g, v);
      ...
      for (String overlayType : overlayEdges.keySet()) {
        if (overlayType.equals("unweighted")) {
          for (Edge edge : overlayEdges.get(overlayType)) {
            paintEdge(g, edge.source, edge.dest, edge.weight, Color.RED, 8, 50);
          }
        }
        if (overlayType.equals("weighted")) {
          for (Edge edge : overlayEdges.get(overlayType)) {
            paintEdge(g, edge.source, edge.dest, edge.weight, Color.GREEN, 8, 50);
          }
        }
    ...
    public void paintVertex(Graphics g, Vertex v) {
      ...
      g2.drawString(v.value, x - v.value.length() * 8 / 2, y + VERTEX_RADIUS / 2);
    }
